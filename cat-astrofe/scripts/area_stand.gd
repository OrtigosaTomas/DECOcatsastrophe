...
func _on_body_entered(body: Node) -> void:
	if body.name.begins_with("decoStand") and body.agarrado == true:
		placeFound = true
	if body.name.begins_with("decoStand") and body.colocado == false:
...
func _process(_delta: float) -> void:
	...
	if controlcajas.standAgarrado == true and areaOcupada == false:
	...
	if not areaOcupada:
		if Input.is_action_just_pressed("interact"):
			for body in bodies_in_area:
				if body.name.begins_with("decoStand") and body.colocado == false and body.agarrado == true:
					print("placed on AreaStand: ", body.name)
